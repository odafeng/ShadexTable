...
export class FileProcessor {
  ...
  static validateFile(
    ...
    try {
      ...
      if (!limits.allowedExtensions.includes(ext)) {
        ...
        reportError(error, {
          fileName: file.name, // 這裡 file 一定存在，所以安全
          extension: ext,
          userType,
      ...
      // 修改這裡：安全地獲取檔案名稱
      reportError(error, {
        fileName: file?.name || "unknown", // 使用可選鏈運算符
  ...
  static checkFileLimits(
    ...
    if (file.size > limits.maxSizeBytes) {
      violations.push(
        `檔案大小 ${this.formatFileSize(file.size)} 超過限制 ${this.formatFileSize(limits.maxSizeBytes)}`,
      );
    }
