...
export default function Step2Page() {
    ...
    const runAnalysis = useCallback(async () => {
        ...
        try {
            ...
            const requestBody = {
                data: activeData,
                group_col: groupVar || null,  // 確保是 null 而不是空字串
                cat_vars: catVars.length > 0 ? catVars : null,  // 空陣列改為 null
                cont_vars: contVars.length > 0 ? contVars : null,  // 空陣列改為 null
                fillNA: fillNA || false,
                enableExport: null,
                enableAI: null,
                correlation_id: currentCorrelationId,
                fileName: useAnalysisStore.getState().fileName || 'unknown.csv',
                fileSize: useAnalysisStore.getState().fileSize || 0,
            };

            // 詳細日誌
            console.log('📊 Request Body Details:');
            console.log('- correlation_id:', currentCorrelationId);
            console.log('- data length:', activeData.length);
            console.log('- group_col:', groupVar);
            console.log('- cat_vars:', catVars);
            console.log('- cont_vars:', contVars);
            console.log('- fillNA:', fillNA);
            console.log('- fileName:', requestBody.fileName);
            console.log('- fileSize:', requestBody.fileSize);
            console.log('- Full request body:', JSON.stringify(requestBody, null, 2));

            const response = await post<typeof requestBody, AnalysisApiResponse>(
            ...
            console.log('✅ 分析完成，correlation ID:', currentCorrelationId);
            ...
            console.error("⚠️ 分析失敗 - 完整錯誤：", error);

            // 如果是 422 錯誤，嘗試解析詳細訊息
            if (error.response?.status === 422) {
                console.error('422 錯誤詳情：', error.response.data);
            }
        ...
        setGroupCounts, setResultTable, correlationId, setCorrelationId]);
    ...
    useEffect(() => {
        // 如果沒有 correlation ID，生成一個新的
        if (!correlation_id) {
            const newcorrelation_id = crypto.randomUUID();
            setcorrelation_id(newcorrelation_id);
            console.log('📌 Generated new correlation ID for Step2:', newcorrelation_id);
        } else {
            console.log('📌 Using existing correlation ID:', correlation_id);
        }
    }, []);
