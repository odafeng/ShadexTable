...
export class ColumnAnalysisService {
    ...
    async analyzeColumns(data: DataRow[], token?: string): Promise<ColumnAnalysisResult> {
    ...
    // 備用方案：創建基本的欄位資訊
    private createFallbackColumnData(data: DataRow[]): ColumnAnalysisResult {
        if (data.length === 0) {
            const error = createError(
    ...
    // 計算遺失值百分比
    private calculateMissingPercentage(data: DataRow[], column: string): string {
        try {
    ...
    // 簡單的類型推斷
    private inferColumnType(data: DataRow[], column: string): string {
        try {
