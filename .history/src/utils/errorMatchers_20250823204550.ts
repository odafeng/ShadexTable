...
export interface ErrorMatchOptions {
  /** 是否檢查 correlation_id 格式 */
  checkcorrelation_id?: boolean;
  /** 是否檢查 timestamp */
  checkTimestamp?: boolean;
  /** 是否檢查 stack trace */
  checkStack?: boolean;
  /** 自訂 correlation_id 格式的正則表達式 */
  correlation_idPattern?: RegExp;
  /** 是否忽略額外的欄位 */
...
const DEFAULT_OPTIONS: ErrorMatchOptions = {
  ...
  checkTimestamp: true,
  checkStack: false,
  correlation_idPattern: /^[a-zA-Z0-9-]+$/,
...
export function expectErrorToMatch(
  ...
  // 提取動態欄位
  const { correlation_id, timestamp, stack, ...staticFields } = error;
  
  // 從預期錯誤中也移除動態欄位（如果存在）
  const { 
    correlation_id: _expectedcorrelation_id, 
  ...
  // 檢查 correlation_id
  if (opts.checkcorrelation_id) {
    expect(correlation_id).toBeDefined();
    expect(typeof correlation_id).toBe('string');
    
    if (correlation_id && opts.correlation_idPattern) {
      expect(correlation_id).toMatch(opts.correlation_idPattern);
    }
  }
...
export function createErrorMatcher(
  ...
  // 處理動態欄位
  if (opts.checkcorrelation_id) {
    matcher.correlation_id = expect.stringMatching(
      opts.correlation_idPattern || DEFAULT_OPTIONS.correlation_idPattern!
    );
  }
