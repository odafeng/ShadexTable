...
export const useBatchVariableUpdate = () => {
  return (updates: {
    ...
    if (updates.fill_na !== undefined) {
      state.setfill_na(updates.fill_na);
    }
  };
};
...
// ========== 自動分析相關 Hooks ==========
export const usecorrelation_id = () =>
  useAnalysisStore(useShallow((state) => ({
    correlation_id: state.correlation_id,
    setcorrelation_id: state.setcorrelation_id,
  })));

// 擴展現有的 useAutoAnalysis hook
export const useAutoAnalysisWithCorrelation = () =>
  useAnalysisStore(useShallow((state) => ({
...
// 用於獲取完整的自動分析狀態
export const useFullAutoAnalysisState = () =>
  useAnalysisStore(useShallow((state) => ({
    autoAnalysisResult: state.autoAnalysisResult,
    skipManualStep: state.skipManualStep,
    ...
    setAutoAnalysisResult: state.setAutoAnalysisResult,
    setSkipManualStep: state.setSkipManualStep,
    setAutoAnalysisMode: state.setAutoAnalysisMode,
    setAiModel: state.setAiModel,
    setcorrelation_id: state.setcorrelation_id,
    clearAutoAnalysis: state.clearAutoAnalysis,
  })));

// 用於生成新的 correlation ID
export const useGeneratecorrelation_id = () => {
  return useCallback(() => {
    const correlation_id = crypto.randomUUID();
    const { setcorrelation_id } = useAnalysisStore.getState();
    setcorrelation_id(correlation_id);
    return correlation_id;
  }, []);
};

// 用於檢查是否有有效的 correlation ID
export const useHascorrelation_id = () =>
  useAnalysisStore((state) => !!state.correlation_id);