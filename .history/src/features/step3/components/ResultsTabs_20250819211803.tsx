...
import type { 
  DndContextProps, 
  DragEndEvent,
  PointerSensor as PointerSensorType,
  KeyboardSensor as KeyboardSensorType,
  UniqueIdentifier,
  CollisionDetection
} from '@dnd-kit/core';
...
const Step3Tabs = memo(function Step3Tabs({
  ...
  const [dndComponents, setDndComponents] = useState<{
    DndContext: React.ComponentType<DndContextProps> | null;
    SortableContext: React.ComponentType<any> | null;
    verticalListSortingStrategy: SortingStrategy | null;
    closestCenter: CollisionDetection | null;
    sensors: any[] | null;
  }>({
    DndContext: null,
  ...
  // 動態載入 DnD 組件和配置
  useEffect(() => {
    if (currentTab === "table" && !dndComponents.DndContext) {
      Promise.all([
        import('@dnd-kit/core'),
  ...
  }, [currentTab, dndComponents.DndContext]);
  ...
  const renderTableContent = () => {
    const { DndContext, SortableContext, verticalListSortingStrategy, closestCenter, sensors } = dndComponents;
    
    if (!DndContext || !SortableContext || !verticalListSortingStrategy || !closestCenter || !sensors) {
      // DnD 載入中顯示的佔位表格
      return (
