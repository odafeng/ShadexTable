...
export function useAnalysisTrigger(): UseAnalysisTriggerReturn {
    ...
    const fill_na = useAnalysisStore(state => state.fill_na);
    ...
    const generateAndSetCorrelationId = useAnalysisStore(state => state.generateAndSetCorrelationId);
    const getCorrelationId = useAnalysisStore(state => state.correlation_id);
    ...
    const handleAutoAnalyze = useCallback(async (file: File) => {
        ...
        const correlation_id = `auto-analysis-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
        ...
        if (fill_na) {
            ...
            console.log('ℹ️ 跳過遺漏值填補（fill_na = false）');
        }
