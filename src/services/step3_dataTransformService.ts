  ...
  try {
    return sortedRows.map((row, idx) => {
      ...
      exportColumns.forEach((col) => {
        if (col === "Variable") {
          let variableName = displayNames[row[col]] || formatVariableName(row[col]);
          ...
          if (isSubItem) {
            ...
            if (prevRow?.Variable?.startsWith("**") && !row[col]?.startsWith("**")) {
              // 檢查是否包含統計描述
          ...
          const label = isGroupCol ? `${groupLabels[col] || col} (n=${groupCounts[col] ?? "?"})` : col;
          ...
          filtered[label] = (value !== "nan" && value !== undefined && value !== null) ? value : "";
        }
      });
      return filtered;
    });
