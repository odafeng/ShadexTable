  ...
  describe('錯誤情境', () => {
    it('應該處理非 2xx 狀態碼並拋出 AppError', async () => {
      ...
      try {
        ...
        expect(appError.correlation_id).toBeDefined()
      }
    })
    ...
    it('應該正確識別並保留已經是 AppError 的錯誤', async () => {
      ...
      const existingAppError = createError(
        ...
        {
          customMessage: '驗證失敗',
          correlation_id: 'existing-id-123'
        }
      )
      ...
      try {
        ...
        expect(appError.correlation_id).toBe('existing-id-123')
      }
    })
  })
})
...
describe('邊界情況測試', () => {
  ...
  it('應該正確產生唯一的 correlation_id', async () => {
    mockFetch.mockResolvedValueOnce({
      ok: false,
      status: 400,
    ...
    expect(errors[0].correlation_id).toBeDefined()
    expect(errors[1].correlation_id).toBeDefined()
    expect(errors[0].correlation_id).not.toBe(errors[1].correlation_id)
  })
...
describe('錯誤物件完整性測試', () => {
  it('應該包含所有必要的 AppError 屬性', async () => {
    ...
    try {
      ...
      expect(appError).toHaveProperty('correlation_id')
      ...
      expect(typeof appError.correlation_id).toBe('string')
